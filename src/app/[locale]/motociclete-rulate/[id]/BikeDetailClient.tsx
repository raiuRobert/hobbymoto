...
interface Props {
  bike: SanityBike;
  similar: SanityBikeCard[];
  locale: string;
}

export default function BikeDetailClient({ bike, similar, locale }: Props) {
  const images = bike.gallery.length > 0 ? bike.gallery : (bike.image ? [bike.image] : []);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-7">
            {images.length > 0 ? (
              ...
              <div className="aspect-[4/3] rounded-sm bg-zinc-900 flex items-center justify-center">
                <p className="text-zinc-600 text-sm">Fără fotografii</p>
              </div>
            )}
          </div>
